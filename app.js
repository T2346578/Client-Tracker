...
function renderDashboard() {
  ...
  const months = Array.from({ length: 12 }, (_, i) => {
    ...
    const key = `${yyyy}-${m}`;
    const t = dashboardTotalsFor(key);
    return { key, ...t };
  });
  ...
  el.innerHTML = `
    <div class="grid gap-4">
      ...
      <div class="card p-5">
        <div>
          <div class="text-sm text-slate-300">Monthly totals (${yyyy})</div>
          <div class="text-xs text-slate-400">Jobs = no tax. Invoices = includes tax & adjustment.</div>
        </div>
          ...
          <table class="w-full min-w-[720px] text-sm">
            <thead class="text-slate-400">
              <tr class="border-b border-white/10">
                <th class="py-2 text-left font-semibold">Month</th>
                <th class="py-2 text-right font-semibold">Jobs</th>
                <th class="py-2 text-right font-semibold">Jobs total</th>
                <th class="py-2 text-right font-semibold">Invoices</th>
                <th class="py-2 text-right font-semibold">Invoices total</th>
              </tr>
            </thead>
            <tbody>
              ${months
                .map(
                  (m) => `
                    <tr class="border-b border-white/5">
                      <td class="py-2">${m.key}</td>
                      <td class="py-2 text-right tabular-nums">${m.jobsCount}</td>
                      <td class="py-2 text-right tabular-nums font-semibold">${moneyFromCents(m.jobRevenue, currency)}</td>
                      <td class="py-2 text-right tabular-nums">${m.invoicesCount}</td>
...
function renderInvoices(routeId) {
  ...
  formWrap.innerHTML = `
    ...
    <div class="mt-4 grid gap-3 sm:grid-cols-2">
      ...
      <div class="sm:col-span-2 mt-2">
        <div class="text-sm font-semibold text-slate-200">Payment (optional)</div>
        <div class="mt-3 grid gap-3 sm:grid-cols-3">
          <div>
            <label class="label">Date</label>
            <input id="payDate" type="date" class="field" />
          </div>
          <div>
            <label class="label">Method</label>
            <input id="payMethod" class="field" placeholder="bank payment" />
          </div>
          <div>
            <label class="label">Amount (${currency})</label>
            <input id="payAmount" type="number" step="0.01" class="field" placeholder="204.94" />
          </div>
          <div class="sm:col-span-3">
            <label class="label">Receipt URL (optional)</label>
            <input id="payReceipt" class="field" placeholder="https://..." />
          </div>
        </div>
      </div>
  ...
  $("#payDate").value = pay?.date || "";
  $("#payMethod").value = pay?.method || "";
  $("#payAmount").value = pay ? moneyInputFromCents(pay.amountCents) : "";
  $("#payReceipt").value = pay?.receiptUrl || "";
  ...
  function readPaymentIntoDraft() {
    ...
    const payAmountCents = centsFromMoneyInput($("#payAmount").value);
    const payReceipt = $("#payReceipt").value;
    const hasAny = payDate || payMethod || $("#payAmount").value || payReceipt;
    if (!hasAny) {
      draft.payments = [];
      return;
    }
    if (!payDate) throw new Error("Payment date is required (or clear all payment fields).");
    if (!Number.isFinite(payAmountCents)) throw new Error("Payment amount is required (or clear all payment fields).");
    draft.payments = [
      {
        date: payDate,
        method: payMethod || "payment",
        amountCents: payAmountCents,
        receiptUrl: payReceipt,
      },
    ];
  }
