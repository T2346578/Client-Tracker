...
function createClient({ name, phone = "", email = "", address = "", notes = "" }) {
...
function createService({ name, priceCents, notes = "" }) {
  const trimmedName = String(name || "").trim();
  if (!trimmedName) throw new Error("Service / job name is required");
  const cents = Number(priceCents);
  if (!Number.isFinite(cents) || cents < 0) throw new Error("Price must be 0 or more");
  return {
    id: newId("svc"),
    name: trimmedName,
...
function formatPaymentMethod(method) {
  ...
  if (lower.includes("bank")) return "E-transfer (bank payment)";
  return raw;
}
...
function renderInvoicePrint({ invoice, client, settings }) {
  ...
  const payLines = payments
    .map((p) => {
      ...
      const method = formatPaymentMethod(p.method || "payment");
        ...
        <div class="mt-8 grid gap-3">
          <div class="rounded-xl border border-slate-200 p-4">
            ...
            <div class="mt-1 text-sm text-slate-700">
              ...
              ${client?.address ? `<div>${safe(client.address)}</div>` : ""}
            </div>
...
function renderDashboard() {
      ...
      <div class="card p-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div class="flex gap-2">
            <button class="btn btn-primary" id="quickAddJobBtn">Add client</button>
  ...
  $("#quickAddJobBtn").onclick = () => navTo("#/clients");
...
function renderClients() {
          ...
          <table class="w-full min-w-[680px] text-sm">
            <thead class="text-slate-400">
              <tr class="border-b border-white/10">
                ...
                <th class="py-2 text-left font-semibold">Phone</th>
                <th class="py-2 text-left font-semibold">Email</th>
                <th class="py-2 text-left font-semibold">Address</th>
  ...
  el.innerHTML = `
    <div class="grid gap-4">
      ...
      <div class="card p-5">
        ...
        <div class="mt-4 grid gap-3 sm:grid-cols-2">
          ...
          <div class="sm:col-span-2">
            ...
            <input id="clientAddress" class="field" placeholder="Customer address for invoices / receipts" />
          </div>
          <div class="sm:col-span-2">
      ...
      <div class="card p-5">
        <div>
          <div class="text-lg font-semibold">Services / Jobs</div>
          <div class="text-sm text-slate-400">
            Keep a library of services and jobs you offer, and quickly add them to invoices.
          </div>
        </div>
        <div class="mt-4">
          ${
            (state.services || []).length === 0
              ? `<div class="text-slate-300">No services yet. Add your first service / job below.</div>`
              : `
                <div class="overflow-x-auto">
                  <table class="w-full min-w-[680px] text-sm">
                    <thead class="text-slate-400">
                      <tr class="border-b border-white/10">
                        <th class="py-2 text-left font-semibold">Name</th>
                        <th class="py-2 text-left font-semibold">Price</th>
                        <th class="py-2 text-left font-semibold">Notes</th>
                        <th class="py-2 text-right font-semibold">Actions</th>
                      </tr>
                    </thead>
                    <tbody>
                        ...
                        .map(
                          (s) => `
                            <tr class="border-b border-white/5">
                              ...
                              <td class="py-2">${moneyFromCents(s.priceCents, currency)}</td>
                              <td class="py-2">${s.notes || "-"}</td>
                              <td class="py-2 text-right">
                                <button class="btn btn-secondary" data-edit-service="${s.id}">Edit</button>
                                <button class="btn btn-danger" data-del-service="${s.id}">Delete</button>
                              </td>
                            </tr>
                          `,
                        )
      ...
      <div class="card p-5">
        <div class="text-lg font-semibold">Add service / job</div>
        <div class="mt-4 grid gap-3 sm:grid-cols-2">
          <div class="sm:col-span-2">
            <label class="label">Name</label>
            <input id="serviceName" class="field" placeholder="Full in & out detail" />
          </div>
          <div>
            <label class="label">Price (${currency})</label>
            <input id="servicePrice" class="field" type="number" step="0.01" placeholder="204.94" />
          </div>
          <div class="sm:col-span-2">
            <label class="label">Notes</label>
            <textarea id="serviceNotes" class="field" rows="3" placeholder="Optional notes"></textarea>
          </div>
          <div class="sm:col-span-2 flex justify-end">
            <button id="addServiceBtn" class="btn btn-primary">Add service / job</button>
          </div>
        </div>
      </div>
    </div>
  ...
  el.querySelectorAll("[data-edit-client]").forEach((btn) => {
    btn.onclick = () => {
      ...
      const updated = { ...c, name: String(name).trim(), phone, email, address, notes };
  ...
  $("#addServiceBtn").onclick = () => {
    try {
      const priceCents = centsFromMoneyInput($("#servicePrice").value);
      if (!Number.isFinite(priceCents)) throw new Error("Price is required");
      const svc = createService({
        name: $("#serviceName").value,
        priceCents,
        notes: $("#serviceNotes").value,
      });
      setState({ ...state, services: [svc, ...(state.services || [])] });
      toast("Service / job added");
    } catch (e) {
      alert(e.message || String(e));
    }
  };

  el.querySelectorAll("[data-del-service]").forEach((btn) => {
    btn.onclick = () => {
      const id = btn.getAttribute("data-del-service");
      setState({ ...state, services: (state.services || []).filter((s) => s.id !== id) });
      toast("Service / job deleted");
    };
  });

  el.querySelectorAll("[data-edit-service]").forEach((btn) => {
    btn.onclick = () => {
      const id = btn.getAttribute("data-edit-service");
      const svc = (state.services || []).find((s) => s.id === id);
      if (!svc) return;
      const name = prompt("Service / job name", svc.name);
      if (name === null) return;
      const priceStr = prompt(`Price (${currency})`, moneyInputFromCents(svc.priceCents));
      if (priceStr === null) return;
      const cents = centsFromMoneyInput(priceStr);
      if (!Number.isFinite(cents) || cents < 0) return alert("Invalid price");
      const notes = prompt("Notes", svc.notes || "");
      if (notes === null) return;
      const updated = {
        ...svc,
        name: String(name).trim() || svc.name,
        priceCents: cents,
        notes,
      };
      setState({
...
function renderInvoices(routeId) {
  ...
  const serviceOptions = services
    .slice()
    .sort((a, b) => a.name.localeCompare(b.name))
    .map((s) => `<option value="${s.id}">${s.name} - ${moneyFromCents(s.priceCents, currency)}</option>`)
    .join("");
          ...
          <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
            ...
            ${
              hasServices
                ? `
                    <div class="flex flex-col gap-2 sm:flex-row sm:items-center">
                      <select id="serviceQuickAdd" class="field sm:w-auto min-w-[220px]">
                        <option value="">Add from services / jobs…</option>
                        ${serviceOptions}
                      </select>
                      <button id="addServiceLineBtn" class="btn btn-secondary">Add to invoice</button>
                    </div>
                  `
                : ""
            }
      ...
      <div class="sm:col-span-2 mt-2">
        ...
        <div class="mt-3 grid gap-3 sm:grid-cols-2">
          <div class="sm:col-span-2">
            ...
            <input id="payMethod" class="field" placeholder="E-transfer (bank payment), cash, cheque" />
  ...
  // Default to "E-transfer (bank payment)" for new invoices to speed up entry,
  // but keep whatever was previously saved when editing.
  $("#payMethod").value = pay?.method || "E-transfer (bank payment)";
  ...
  const serviceQuickAdd = $("#serviceQuickAdd");
  const addServiceLineBtn = $("#addServiceLineBtn");
  if (serviceQuickAdd && addServiceLineBtn) {
    addServiceLineBtn.onclick = () => {
      const svcId = serviceQuickAdd.value;
      if (!svcId) {
        alert("Select a service / job to add");
        return;
      }
...
$("#exportBtn").onclick = async () => {
  // Detect if the invoice print view is actually visible
  const printView = $("#view-invoice-print");
  const isPrintVisible = printView && !printView.classList.contains("hidden");
  const invId = currentPrintInvoiceId;

  // When viewing a printable invoice, export it as a PDF instead of JSON
  if (isPrintVisible && invId) {
    const printEl = document.querySelector("#view-invoice-print .print-sheet") || printView;
    if (!printEl) {
      toast("No invoice to export");
      return;
    }

    const inv = byId(state.invoices, invId);
    const invNo = inv && (inv.invoiceNumber || inv.invoiceNumber === 0) ? inv.invoiceNumber : null;
    const filename = invNo ? `invoice-${invNo}.pdf` : `invoice-${invId}.pdf`;

    if (typeof html2pdf === "undefined") {
      alert("PDF export library failed to load. Try using your browser's Share / Print to PDF.");
      return;
    }
    ...
    try {
      ...
      alert("Couldn't export invoice as PDF. You can still use your browser's Print / Share as PDF option.");
    }

    return;
  }

  // Default: export JSON backup of all data
